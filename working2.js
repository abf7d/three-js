...
function animate() {
  ...
  if (animationPhase === 'expand') {
    progress = Math.min(elapsed / 0.3, 1); // Progress from 0 to 1 over 1 second (faster)
    ...
    progress = 1 - Math.min(elapsed / 0.3, 1); // Progress from 1 to 0 over 1 second (faster)
    ...
    if (elapsed >= 2) {
      // Pause for 3 seconds before expanding again
  ...
  polygons.forEach((polygon, index) => {
    if (index === 1) {
      // Only animate the center polygon
